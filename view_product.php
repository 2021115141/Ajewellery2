        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-12 col-sm-6">
              ...
              <p>Wages: <?php echo ucwords($wages) ?></p>
              <hr>
              ...
              <h4>Available Weight</h4>
              <?php 
              $weights = $conn->query("SELECT * FROM weights where product_id = $id");
              $weight_arr = array();
              while($row = $weights->fetch_assoc()){
              	$weight_arr[$row['id']] = $row['weight'];
              }
              ?>
              <div class="form-group">
                <select name="weight_id" id="weight_id" class="custom-select">
                  <?php if(count($weight_arr) == 0): ?>
                    <option value="0">N/A</option>
                  <?php else: ?>
                  <?php foreach($weight_arr as $k => $v): ?>
                    <option value="<?php echo $k ?>"><?php echo $v ?></option>
                  <?php endforeach; ?>
                  <?php endif; ?>
                </select>
              ...
              <?php
              $type = $qry['type'];
              $qry2 = $conn->query("SELECT * FROM setting s where s.setting_type = '$type' AND s.setting_date = CURDATE() ")->fetch_array();
              
              
              ?>
              
              <div class="bg-dark disabled py-2 px-3 mt-4">
                <h4 class="mb-0">
                  Price per Gram for Today</br>
                  <?php echo number_format($qry2['setting_new'],2) ?>
                </h4>
  ...
  $(document).ready(function(){
      ...
      $('#add_to_cart').click(function(){
        ...
        $.ajax({
            ...
            method:'POST',
            data:{product_id: '<?php echo $id ?>',price: '<?php echo $qry2['setting_new'] ?>', qty:$('#qty').val(), colour_id:$('#colour_id').val(), size_id:$('#size_id').val(),weight_id:$('#weight_id').val()},
