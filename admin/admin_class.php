...
Class Action {
	...
	function add_to_cart(){
		...
		$data .= ", qty= $qty";
		$data .= ", price= $price";
		$check = $this->db->query("SELECT * FROM cart where product_id = $product_id and colour_id = $colour_id and weight_id = $weight_id and size_id = $size_id  and user_id ={$_SESSION['login_id']} ");
	...
	function save_product(){
		...
		$data = "";
		foreach($_POST as $k => $v){
			if(!in_array($k, array('id','dname','fname','size','sid','color','cid','item_code','weight','wid')) && !is_numeric($k)){
				if($k == 'description')
			...
			if(isset($weight)){
				foreach ($weight as $k => $v) {
					if(!empty($weight)){
						...
						$data .= ", weight = '$v' ";
						if($wid[$k] > 0){
							$this->db->query("UPDATE weights set $data where id = {$wid[$k]}");
						}else{
							$this->db->query("INSERT INTO weights set $data");
							$wid[$k] = $this->db->insert_id;
						}
					}
				}
			}
			if(isset($wid)){
				$wids = array_filter($wid);
				if(count($wids) > 0){
					$wids = implode(',', $wids);
					$this->db->query("DELETE FROM weights where id not in ($wids) and product_id = $id ");
				}
			}
			if(isset($color)){
	...
	function save_setting(){
		extract($_POST);
		if($id == null)
		$save = $this->db->query("INSERT INTO setting (setting_date,setting_type, setting_new, setting_used) VALUES ('$date','$type','$new','$used')");
		else
		$save = $this->db->query("UPDATE setting SET setting_date = '$date',setting_type = '$type', setting_new = '$new', setting_used = '$used' WHERE id =".$id);
	...
	function delete_setting(){
		extract($_POST);
		$prod = $this->db->query("SELECT * from setting where id = $id")->fetch_array();
		$delete = $this->db->query("DELETE FROM setting where id = $id");
		if($delete){
			return 1;
		}
	}
	...
	function update_setting(){
		extract($_POST);
		$save = $this->db->query("UPDATE orders setting = $status where id = $id");
		if($save)
			return 1;
