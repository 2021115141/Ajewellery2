    		...
    		<ul class="list-group">
    			...
    			while($row= $qry->fetch_array()):
    				...
    				$colour = $colour->num_rows>0 ? $colour->fetch_array()['color'] : 'N/A';
             		$weights = $conn->query("SELECT * FROM weights where id = {$row['weight_id']}");
             		$weights = $weights->num_rows>0 ? $weights->fetch_array()['weight'] : 'N/A';
					$wages = $conn->query("SELECT * FROM products where id = {$row['product_id']}");
             		$wages = $wages->num_rows>0 ? $wages->fetch_array()['wages'] : 'N/A';
					$total += $row['qty']*($row['price']*$weights+$wages);
    				$img = array();
    			...
    			?>
    			<li class="list-group-item" data-id="<?php echo $row['id'] ?>" data-price="<?php echo $row['price'] ?>"data-weight="<?php echo $weights ?>"data-wages="<?php echo $wages ?>">
    				<div class="d-flex w-100">
    					...
    					<div class="detail-field">
							...
							<p>Weight: <b><?php echo $weights ?></b></p>
							<p>Wages: <b><?php echo $wages ?></b></p>
    					...
    					<div class="amount-field">
    						<b class="amount"><?php echo number_format($row['qty']*$row['price']*$weights+$wages,2) ?></b>
    					</div>
	...
	$('.btn-minus').click(function(){
     ...
     function calc(){
     	$('.qty-input').each(function(){
			...
			var weight = li.attr('data-weight')
			var wages = li.attr('data-wages')
     		var qty = $(this).val()
     		var amount = parseFloat(qty) * (parseFloat(price) * parseFloat(weight)+parseFloat(wages));
